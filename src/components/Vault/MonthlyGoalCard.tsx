...
export function MonthlyGoalCard({
  ...
  const monthName = goal ? MONTH_NAMES[goal.month - 1] : ''
  const target = goal?.targetAmount ?? 0
  const current = goal?.currentAmount ?? 0
  const percentage = calculatePercentage(current, target)
  const remaining = Math.max(target - current, 0)
  const isCompleted = goal ? goal.completed || current >= target : false
    ...
    <div className="glass-card rounded-2xl p-6 border border-zinc-800/80 bg-zinc-900/50">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h3 className="text-lg font-bold text-white">
              {goal ? `Meta de ${monthName} de ${goal.year}` : 'Meta Mensal'}
            </h3>
          </div>
        </div>
      ...
      <div className="space-y-4 pt-4">
        {goal ? (
          ...
          <div className="bg-zinc-950/60 p-8 rounded-xl border border-dashed border-zinc-800 text-center">
            <p className="text-sm text-zinc-400 mb-4">
              Nenhuma meta mensal definida ainda.
            </p>
            <Button variant="outline" onClick={onEditGoal} className="gap-1.5 text-xs">
